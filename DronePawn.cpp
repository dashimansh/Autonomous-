...
void ADronePawn::BeginPlay()
{
	...
	if (PC)
	{
		...
		if (DroneHUD && PiPRenderTarget)
			DroneHUD->SetPiPRenderTarget(PiPRenderTarget);
	}

	if (GEngine)
		GEngine->AddOnScreenDebugMessage(5, 5.f, FColor::Yellow,
			TEXT("W/S=Fwd A/D=Strafe E/Q=Alt Z/X=Yaw "
				"1=Motor 2=Flight 3=Mode 4=Input "
				"H=HUD M=Map N=ArmLights V=Body R=RTL"));
}
...
void ADronePawn::ApplyRTLFlight(float DeltaTime)
{
	...
	if (bRTLFollowingPath)
	{
		...
		if (Dist > RTLAcceptRadius)
		{
			...
			float AltErr = Target.Z - Pos.Z;
			DroneVelocity.Z = FMath::Clamp(
				AltErr * 0.5f, -RTLSpeed, RTLSpeed);
		}
		else
		{
			CurrentRTLIndex++;
			if (GEngine) GEngine->AddOnScreenDebugMessage(
				11, 0.5f, FColor::Cyan,
				FString::Printf(TEXT("RTL: Point %d / %d"),
...
void ADronePawn::ApplyWaypointFlight(float DeltaTime)
{
	...
	float   TargZ = WaypointAltitude;
	DroneVelocity.Z = FMath::Clamp(
		(TargZ - Pos.Z) * 0.5f, -WaypointSpeed, WaypointSpeed);
	...
	else
	{
		...
		else
		{
			bWaypointMode = false;
			bLoiterMode = true;
			LoiterPosition = FVector(Target.X, Target.Y, TargZ);
			DroneVelocity = FVector::ZeroVector;
			PID_Alt.Reset();
			if (GEngine) GEngine->AddOnScreenDebugMessage(
				12, 4.f, FColor::Green,
				TEXT(">> ALL WAYPOINTS COMPLETE <<"));
		}
	}
	FVector NewPos = Pos + DroneVelocity * DeltaTime;
	CheckGroundCollision(NewPos);
...
{
	if (!GimbalCamera || !bGimbalActive) return;
...
void ADronePawn::DrawWaypoints()
{
	...
	for (int32 i = 0; i < Waypoints.Num(); i++)
	{
		...
		DrawDebugSphere(GetWorld(), WPTop, 100.f, 16,
			WPC, false, 0.05f, 0, 4.f);
		DrawDebugLine(GetWorld(),
			FVector(WP.X, WP.Y, 0.f), WPTop,
			WPC, false, 0.05f, 0, 3.f);
		if (i < Waypoints.Num() - 1)
		{
			FVector NTop = FVector(
				Waypoints[i + 1].X, Waypoints[i + 1].Y,
				WaypointAltitude);
			DrawDebugLine(GetWorld(), WPTop, NTop,
...
{
	if (!bJoystickMode || bLoiterMode || bRTLMode || bWaypointMode)
		return;
...
void ADronePawn::OnToggleGimbal()
{
	...
	if (bGimbalActive)
	{
		...
		if (GEngine) GEngine->AddOnScreenDebugMessage(
			40, 2.f, FColor::Cyan,
			TEXT(">> GIMBAL ON  I/K=Pitch  J/L=Yaw <<"));
	}
	else
		if (GEngine) GEngine->AddOnScreenDebugMessage(
...
void ADronePawn::OnRTLToggle()
{
	...
	if (bRTLMode)
	{
		...
		if (GEngine) GEngine->AddOnScreenDebugMessage(
			...
			FString::Printf(
				TEXT(">> RTL ACTIVE — RETRACING %d TRAIL POINTS <<"),
				RTLPath.Num()));
	}
	else
	{
